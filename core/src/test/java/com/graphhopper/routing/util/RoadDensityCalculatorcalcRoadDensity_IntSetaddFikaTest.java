...
public class RoadDensityCalculatorcalcRoadDensity_IntSetaddFikaTest {
    ...
    public void testCalcRoadDensity() {
        ...
        Mockito.when(nodeAccess.getLat(1)).thenReturn(0.0);
        Mockito.when(nodeAccess.getLon(1)).thenReturn(0.0);
        Mockito.when(nodeAccess.getLat(2)).thenReturn(0.0);
        Mockito.when(nodeAccess.getLon(2)).thenReturn(0.0);
        ...
        ToDoubleFunction<EdgeIteratorState> calcRoadFactor = value -> 1.0;
        
        calculator.calcRoadDensity(edgeIteratorState, 100.0, calcRoadFactor);
    }
}
